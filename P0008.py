...
data_numbers = [7, 3, 1, 6, 7, 1, 7, 6, 5, 3, 1, 3, 3, 0, 6, 2, 4, 9, 1, 9, 2, 2, 5, 1, 1, 9, 6, 7, 4, 4, 2, 6, 5, 7, 4,
...
def compute_product(index):
    computed_product = 1
    if index + 5 > 999:
        for i in range(index, 999):
            computed_product *= data_numbers[i]
    else:
        for i in range(index, index + 5):
            computed_product *= data_numbers[i]

    return computed_product

if __name__ == '__main__':
    results = Parallel(n_jobs=-1)(delayed(compute_product)(n) for n in range(999))
    print(results)
    print(max(results))
